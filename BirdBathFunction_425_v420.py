...
def BirdbathFunc425( Harry, Dumbledore, Sirius ) :
    Snuffleupagus   = np.array( [ -204e-9, 20e-9, 427.854e-6, 999.87597e-3, 995.41971e-2  ] )
    Susan           = np.array( [  0.98488, -0.59685, -0.89071 ] )
    Ernie           = np.array( Harry )
    Bob             = np.array( [  0.03654,  0.78435, -0.43852 ] )
    Troll           = np.polyval( Snuffleupagus, Ernie )
    Hooper          = np.array( [ -0.16935,  0.16903,  0.11977 ] )
...
if __name__ == '__main__' :
    ...
    print('\n\n\n')
    nn  = BirdbathFunc425( 9.7500, 2.1250, 15.0430 )
